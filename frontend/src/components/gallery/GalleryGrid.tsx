...
const fallbackImages: Record<string, string> = {
  general: 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=900&q=85',
  workspace: 'https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=900&q=85',
  events: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&w=900&q=85',
  projects: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=900&q=85',
  facility: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=900&q=85',
};
...
const GalleryGrid = ({ items, className = '' }: GalleryGridProps) => {
  ...
  const getFallbackImage = (category: string) => fallbackImages[category] || fallbackImages.general;
  const getImageUrl = (imageUrl: string, category: string) => getMediaUrl(imageUrl) || getFallbackImage(category);
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredItems.map((item, index) => (
            <button key={item.id} type="button" onClick={() => setSelectedItem(item)} className={`group relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-100 text-left shadow-card transition duration-300 hover:-translate-y-1 hover:border-slate-300 hover:shadow-card-hover ${index === 0 ? 'sm:row-span-2 sm:min-h-[520px]' : 'min-h-[250px]'}`}>
              <img src={getImageUrl(item.image, item.category)} alt={item.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" onError={event => { event.currentTarget.src = getFallbackImage(item.category); }} />
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={selectedItem.title} onClick={() => setSelectedItem(null)}>
          <div className="relative max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={event => event.stopPropagation()}>
            ...
            <img src={getImageUrl(selectedItem.image, selectedItem.category)} alt={selectedItem.title} className="max-h-[72vh] w-full object-contain bg-slate-100" onError={event => { event.currentTarget.src = getFallbackImage(selectedItem.category); }} />
