  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {filteredProducts.length > 0 ? <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{filteredProducts.map(product => <ProductCard key={product.id} id={product.id.toString()} name={product.name} price={product.price} image={product.image_url || ''} category={product.category} inStock={product.is_available && product.stock_quantity > 0} />)}</div> : <div className="mt-10 rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-6 py-20 text-center"><SearchIcon className="mx-auto text-slate-400" size={28} /><h2 className="mt-4 font-grotesk text-2xl font-bold text-slate-900">No products found</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Try another search or reset the catalogue filters.</p><button onClick={resetFilters} className="btn btn-outline mt-6">Reset filters</button></div>}
      </section>
